...
require_once __DIR__ . "/analytics-final-validation.php";
...
if($expected === "" || $csrf === "" || !hash_equals($expected, $csrf)){
    http_response_code(403);
    echo json_encode(["ok" => false, "message" => "Invalid validation token."]);
    exit;
}
...
$environment = analyticsMetricsEnvironment($payload["environment"] ?? analyticsCurrentEnvironment());
$result = analyticsFinalValidationBuild($connection, $environment);

echo json_encode(
    [
        "ok" => true,
        "message" => "Validación integral completada.",
        "result" => $result
    ],
